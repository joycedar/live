...
extension UILabel{
    ...
    func setLableSpacing(_ spacing:CGFloat,content:String?) {
        ...
        attStr.addAttribute(NSAttributedString.Key.paragraphStyle, value: style, range: NSRange.init(location: 0, length: (content?.count)!))
    ...
    func getNumOfLineWithContent(_ content:String,width:CGFloat) -> NSInteger {
        ...
        let rect = (content as NSString).boundingRect(with: CGSize.init(width: width, height: CGFloat(MAXFLOAT)), options: NSStringDrawingOptions.usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font:self.font], context: nil)
