...
class KKLMyLiveView: UIView,LFLiveSessionDelegate {
    ...
    private func requestAccessForAudio(){
        weak var wself = self
        let status:AVAuthorizationStatus = AVCaptureDevice.authorizationStatus(for: AVMediaType.audio)
        switch status {
        case AVAuthorizationStatus.notDetermined:
            // 许可对话没有出现，发起授权许可
    ...
    private func dealEvent(){
        ...
        closeButton.addBlock(for: UIControl.Event.touchUpInside) { (sender) in
            wself?.stopLive()
            wself?.vc?.dismiss(animated: true, completion: {})
        }
        
        cameraButton.origin = CGPoint.init(x: closeButton.left - 10 - cameraButton.width, y: 20)
        cameraButton.addBlock(for: UIControl.Event.touchUpInside) { (sender) in
            //切换前后摄像头的方法
            let devicePositon:AVCaptureDevice.Position = (wself?.session.captureDevicePosition)!
            wself?.session.captureDevicePosition = (devicePositon == AVCaptureDevice.Position.back) ? AVCaptureDevice.Position.front : AVCaptureDevice.Position.back
        }
        
        beautyBtn.origin = CGPoint.init(x: cameraButton.left - 10 - cameraButton.width, y: 20)
        beautyBtn.addBlock(for: UIControl.Event.touchUpInside) { (sender) in
    ...
    var closeButton:UIButton = {
        ...
        btn.left = UIScreen.main.bounds.width - 10 - btn.width
        btn.top = 20
        btn.setImage(UIImage.init(named: "close_preview"), for: UIControl.State.normal)
        return btn
    }()
    ...
    var cameraButton:UIButton = {
        let btn = UIButton.init(type: UIButton.ButtonType.custom)
        btn.size = CGSize.init(width: 44, height: 44)
        btn.setImage(UIImage.init(named: "camra_preview"), for: UIControl.State.normal)
        return btn
    }()
    ...
    var beautyBtn:UIButton = {
        let btn = UIButton.init(type: UIButton.ButtonType.custom)
        btn.size = CGSize.init(width: 44, height: 44)
        btn.setImage(UIImage.init(named: "camra_beauty"), for: UIControl.State.normal)
        btn.setImage(UIImage.init(named: "camra_beauty_close"), for: UIControl.State.selected)
        return btn
    }()
