...
class KKLBarrageView: UIView {
    ...
    private func createLable(contentStr:String,type:@escaping KKLBarrageLableTypeBlock){
        ...
        let barrageSize = contentStr.size(withAttributes:[NSAttributedString.Key.font : UIFont.systemFont(ofSize: 14)])
        barrageLable.size = CGSize.init(width: barrageSize.width + 20, height: lableH)
        
        //计算运动总长度
        let moveW = barrageLable.width + UIScreen.main.bounds.width
        let moveT:CGFloat = 8
        ...
        barrageLable.left = UIScreen.main.bounds.width
