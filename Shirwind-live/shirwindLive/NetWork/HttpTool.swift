...
class HttpTool: NSObject {
     ...
     */
    class func getWithPath(path:String,params:NSDictionary?,
                           success:@escaping HttpSuccessBlock,
                           failure:@escaping HttpFailureBlock) {
        let allUrl = SERVER_HOST + "/" + path
        AFHttpClient.sharedClient.get(allUrl, parameters: params, headers: nil, progress: nil, success: { (task, responseObject) in
    ...
    class func postWithPath(path:String,params:NSDictionary,success:@escaping HttpSuccessBlock,failure:@escaping HttpFailureBlock) {
        let allUrl = SERVER_HOST + "/" + path
        AFHttpClient.sharedClient.post(allUrl, parameters: params, headers: nil, progress: nil, success: { (task, responseObject) in
            success(responseObject)
        }) { (task, error) in
            failure(error as NSError)
        }
...
class AFHttpClient:AFHTTPSessionManager{
    //创建单例
    private static let httpClient = AFHttpClient(baseURL: NSURL(string: SERVER_HOST) as URL?,
                                                 sessionConfiguration: URLSessionConfiguration.default)
    
    class var sharedClient:AFHttpClient {
        httpClient.responseSerializer.acceptableContentTypes = NSSet(array: ["application/json","text/html", "text/json", "text/javascript","text/plain","image/gif"]) as? Set<String>
